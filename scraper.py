...
def get_page_source(browser_path, url):
    """ get the input page source code
        download firefox browser from https://github.com/mozilla/geckodriver/releases
...
def composite_url(location, seller_type, search_term, page_num):
    search_path = '%20'.join(search_term.split())
    base_path = "https://{}.craigslist.org/search/{}?s={}&query={}"
    url = base_path.format(location, seller_type, page_num, search_path)
    return url

def get_all_item_url(browser_path, location, seller_type, search_term):
    ...
    """
    first_page_url = composite_url(location, seller_type, search_term, 0)
    first_page_soup = get_page_source(browser_path, first_page_url)

    total_item = get_total_item(first_page_soup)
    page_source_num = [i+1 for i in range(0, total_item, 120) if i != 0]

    # first page url
    first_page_url = get_page_item_url(first_page_soup)

    if len(page_source_num) > 0:
        # all page link
        all_but_one_link = [composite_url(location, seller_type, search_term, i) for i in page_source_num]
        # all item url but page one
        all_item_url_but_one = [get_page_item_url(get_page_source(browser_path, i)) for i in all_but_one_link]
...
def extract_page_info(browser_path, url):
    """ extract all info within an input url

    arg: browser_path (str) - the path where browser is located
         url (str) - the page link

    return: all_info (dict) - a dictionary of all info
    """
    try:
        soup = get_page_source(browser_path, url)
...
class CraigslistExtractor:
    def __init__(self, browser_path, location, seller_type, search_term):
        self.browser_path = browser_path
        self.location = location
        self.seller_type = seller_type
        self.search_term = search_term

        self.all_item_url = get_all_item_url(self.browser_path, self.location, self.seller_type, self.search_term)
        self.total_extracted_item = len(self.all_item_url)

    def get_all_item_info(self, n=None, random=False, seed=None, save_data=False, file_name=None, return_=True):
        ...
        arg: n (int) - extract how many result
             ...
             save_data (boolean) - save data to a .csv file or not, optional, default is False
             file_name (str) - if save_data is True, the file name you want to save, optional, default is None
                               if None, will use the search_term to name the file
             return_ (boolean) - return or not, optional, default is True
             
        return: (dataframe) - a dataframe with all the info
        """
        if n is None:
            all_info = [extract_page_info(self.browser_path, i) for i in tqdm(self.all_item_url)]
            ...
            all_info = [extract_page_info(self.browser_path, i) for i in tqdm(np.array(self.all_item_url)[random_idx])]
        elif (n is not None) and (random is False):
            all_info = [extract_page_info(self.browser_path, i) for i in tqdm(self.all_item_url[:n])]

        df = list_to_df(all_info)

        if save_data:
            if file_name is None:
                df.to_csv(self.search_term.replace(' ','_')+'.csv', index=False)
            else:
